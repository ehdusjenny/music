...
def collate_fn(batch):
    ...
    if isinstance(elem, torch.Tensor):
		# --------------------------------------------------
        shapes = torch.tensor([e.shape for e in batch])
        max_shape = shapes.max(0).values
        output = [torch.zeros(max_shape.numpy().tolist()) for _ in batch]
        for o,e in zip(output,batch):
            s = [slice(0,l) for l in e.shape]
            o[s] = e
        batch = output
		# --------------------------------------------------
        out = None
